...
func (w *WinAPI) Receive() *TunnelData {
	...
	if (t.dataType & DataInVendor) != 0 {
		t.dataType &= ^DataInVendor
		entry := blist.wlanBssEntries[entryID]
		pEntry := unsafe.Pointer(&blist.wlanBssEntries[entryID])
		IEs := []byte{}
		for i := uint32(0); i < entry.ulIeSize; i++ {
			dat := *(*byte)(unsafe.Pointer(uintptr(pEntry) + uintptr(entry.ulIeOffset) + uintptr(i)))
			IEs = append(IEs, dat)
		}
		...
		t.payload = append(t.payload, vendor[2:]...)
	}
	return t
}
